...
export default function ScrolableHorizontal() {
  ...
  const columns = [
    ...
    new ColumnModel(
      ...
      false,
      1,
      board_data[player_id]["expectedValues"][0]
    ),
    new ColumnModel(
      "inprog",
      "In Progress",
      ...
      false,
      1,
      board_data[player_id]["expectedValues"][1]
    ),
    new ColumnModel(
      "done",
      "Done",
      ...
      false,
      1,
      board_data[player_id]["expectedValues"][2]
    ),
  ];
  const cards = [];
  for (let i = 0; i < 7; i++) {
  ...
  const onCardDragEnd = async (
    ...
    console.log(results);
    if (
      results[0] === board_data[player_id]["expectedValues"][0] &&
      results[1] === board_data[player_id]["expectedValues"][1] &&
      results[2] === board_data[player_id]["expectedValues"][2]
    ) {
      console.log("Solved!");
      ...
      Alert.alert("Gratulacje! Wygrałeś!", "", [
        {
          text: "OK",
          onPress: () => {
            setCompleted((completed) => [...completed, "KANBAN"]);
